"""
第3章：V5 风格课程学习训练脚本（UniformRodPlatform3DOF 版本）

设计原则：
1. 奖励函数采用 V5 Simplified 的纯 ISE 目标
2. 保留 V3 的三阶段 curriculum，降低直接训练 30s/35s 任务难度
3. 支持在 Stage 3 打开 burst-step 突变扰动做鲁棒性训练
4. 使用当前已验证可用的 AdvancedSAC，避免额外 agent 兼容性风险
"""
...
class CurriculumConfigV5Uniform:
    """V5-style + curriculum + UniformRod 的训练配置。"""

    def __init__(self):
        self.stage1 = {
            'name': 'Stage1_V5Uniform_Basic',
            'episode_length': 10,
            'dt': 0.01,
            'max_episodes': 150,
            'Hs': 1.0,
            ...
            'save_dir': 'models_v5_uniform/stage1'
        }
        self.stage2 = {
            'name': 'Stage2_V5Uniform_Tracking',
            'episode_length': 20,
            'dt': 0.01,
            'max_episodes': 250,
            'Hs': 2.0,
            'T1': 8.0,
            'q_des_type': 'sinusoidal_small',
            'enable_burst_step': False,
            'save_dir': 'models_v5_uniform/stage2'
        }
        self.stage3 = {
            'name': 'Stage3_V5Uniform_FullTask',
            'episode_length': 30,
            'dt': 0.01,
            'max_episodes': 400,
            'Hs': 2.0,
            ...
            'step_ramp_time': 0.03,
            'save_dir': 'models_v5_uniform/stage3'
        }

    def make_quick_test(self):
        quick = CurriculumConfigV5Uniform()
        quick.stage1.update({
            'name': 'Stage1_V5Uniform_Quick',
            'episode_length': 2,
            'max_episodes': 2,
            'save_dir': 'models_v5_uniform_quick/stage1'
        })
        quick.stage2.update({
            'name': 'Stage2_V5Uniform_Quick',
            'episode_length': 2,
            'max_episodes': 2,
            'enable_burst_step': False,
            'save_dir': 'models_v5_uniform_quick/stage2'
        })
        quick.stage3.update({
            'name': 'Stage3_V5Uniform_Quick',
            'episode_length': 2,
            'max_episodes': 2,
            'enable_burst_step': True,
            'step_t0': 0.8,
            'step_duration': 0.2,
            'save_dir': 'models_v5_uniform_quick/stage3'
        })
        return quick
...
def train_stage(agent, env_config, stage_label, load_model=None, warmup_steps=5000, batch_size=512):
    print(f"\n{'=' * 72}")
...
def train_curriculum_v5_uniform():
    print("\n" + "=" * 72)
    print("第3章 V5-Style Curriculum Training (UniformRod + AdvancedSAC)")
    print("=" * 72)

    config = CurriculumConfigV5Uniform()
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    agent = build_agent(device=device)

    print(f"训练设备: {device}")
    print(f"网络结构: {agent.actor.layers}")
    ...
    run_dir = Path(__file__).parent / 'models_v5_uniform'
    run_dir.mkdir(parents=True, exist_ok=True)

    overall = {
        'variant': 'V5_Uniform_Curriculum',
        'started_at': datetime.now().isoformat(),
        'device': device,
        'stages': []
    }

    agent, history1, summary1 = train_stage(agent, config.stage1, 'Stage1_V5Uniform', load_model=None)
    overall['stages'].append(summary1)

    stage1_best = str(Path(__file__).parent / config.stage1['save_dir'] / 'Stage1_V5Uniform_best.pt')
    agent, history2, summary2 = train_stage(agent, config.stage2, 'Stage2_V5Uniform', load_model=stage1_best)
    overall['stages'].append(summary2)

    stage2_best = str(Path(__file__).parent / config.stage2['save_dir'] / 'Stage2_V5Uniform_best.pt')
    agent, history3, summary3 = train_stage(agent, config.stage3, 'Stage3_V5Uniform', load_model=stage2_best)
    ...
    print("\n" + "=" * 72)
    print("V5 Uniform Curriculum 全部完成")
    print("=" * 72)
...
def train_curriculum_v5_uniform_quick():
    print("\n" + "=" * 72)
    print("第3章 V5 Uniform Quick Test")
    print("=" * 72)

    config = CurriculumConfigV5Uniform().make_quick_test()
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    agent = AdvancedSAC(
        state_dim=9,
        action_dim=3,
    ...
    agent, _, _ = train_stage(agent, config.stage1, 'Stage1_V5Uniform_Quick', warmup_steps=10, batch_size=16)
    stage1_best = str(Path(__file__).parent / config.stage1['save_dir'] / 'Stage1_V5Uniform_Quick_best.pt')
    agent, _, _ = train_stage(agent, config.stage2, 'Stage2_V5Uniform_Quick', load_model=stage1_best, warmup_steps=10, batch_size=16)
    stage2_best = str(Path(__file__).parent / config.stage2['save_dir'] / 'Stage2_V5Uniform_Quick_best.pt')
    agent, _, _ = train_stage(agent, config.stage3, 'Stage3_V5Uniform_Quick', load_model=stage2_best, warmup_steps=10, batch_size=16)
    print('Quick test completed.')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train V5-style Uniform curriculum SAC')
    parser.add_argument('--quick', action='store_true', help='run a tiny smoke/quick test')
    args = parser.parse_args()

    if args.quick:
        train_curriculum_v5_uniform_quick()
    else:
        train_curriculum_v5_uniform()