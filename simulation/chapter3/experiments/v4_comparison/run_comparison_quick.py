"""
V4 Improved变体快速对比评估脚本（跳过不支持的参数）
"""
...
def compute_ise_metrics(env, agent, n_episodes=3, seed=None):
    """计算ISE指标"""
    if seed is not None:
        np.random.seed(seed)
    ...
    return {
        ...
        'ise_per_step_std': np.sum(std_ise) / max(avg_steps, 1),
...
def main():
    ...
    agent = V4SAC(state_dim=9, action_dim=3, hidden_dims=[512, 512, 256, 256], device=None)
    ...
    variants = {
        ...
        'Original_V4_Improved': {
            ...
            'extra_params': {'diverge_threshold': 0.5, 'warning_threshold': 0.4}
        },
        'Variant1_Relaxed': {
            'env_class': PlatformRLEnvV4Improved,
            'extra_params': {'diverge_threshold': 1.0, 'warning_threshold': 0.4}
        },
    }
    ...
    for scenario in test_scenarios:
        ...
        for variant_name, variant_config in variants.items():
            ...
            for traj_type in trajectory_types:
                ...
                print(f"  {traj_type:20s}: ISE/step = {avg_ise:.6f} ± {std_ise:.6f}")
    ...
    result_file = results_dir / f'comparison_quick_{timestamp}.json'
