"""
V5 Fixed SAC - 修复BatchNorm问题
改为LayerNorm，其他保持V5优化
"""
...
class V5FixedActor(nn.Module):
    """V5 Actor - 使用LayerNorm"""
    ...
    def __init__(self, state_dim, action_dim, 
        ...
        # 输入层
        self.input_layer = orthogonal_init(nn.Linear(state_dim, hidden_dims[0]))
        self.input_ln = nn.LayerNorm(hidden_dims[0])  # LayerNorm！
        
        # 隐藏层
        self.fc1 = orthogonal_init(nn.Linear(hidden_dims[0], hidden_dims[1]))
        self.ln1 = nn.LayerNorm(hidden_dims[1])
        
        self.fc2 = orthogonal_init(nn.Linear(hidden_dims[1], hidden_dims[2]))
        self.ln2 = nn.LayerNorm(hidden_dims[2])
        
        self.fc3 = orthogonal_init(nn.Linear(hidden_dims[2], hidden_dims[3]))
        self.ln3 = nn.LayerNorm(hidden_dims[3])
        
        # 残差连接
        self.residual1 = orthogonal_init(nn.Linear(hidden_dims[0], hidden_dims[1]))
        self.residual2 = orthogonal_init(nn.Linear(hidden_dims[1], hidden_dims[2]))
        self.residual3 = orthogonal_init(nn.Linear(hidden_dims[2], hidden_dims[3]))
        
        self.dropout = nn.Dropout(dropout)
        
        # 输出层
        self.mean = orthogonal_init(nn.Linear(hidden_dims[3], action_dim), gain=0.01)
        self.log_std = orthogonal_init(nn.Linear(hidden_dims[3], action_dim), gain=0.01)
    
    def forward(self, state):
        # 输入层
        x = self.input_layer(state)
        x = self.input_ln(x)  # LayerNorm！
        x = F.relu(x)
        x = self.dropout(x)
...
class V5FixedCritic(nn.Module):
    """V5 Critic - 使用LayerNorm"""
    ...
    def __init__(self, state_dim, action_dim,
        ...
        # 输入层
        self.input_layer = orthogonal_init(nn.Linear(input_dim, hidden_dims[0]))
        ...
        # 输出层
        self.q = orthogonal_init(nn.Linear(hidden_dims[3], 1), gain=1.0)
...
class ReplayBuffer:
    ...
    def __init__(self, state_dim, action_dim, capacity=2000000):
...
class V5FixedSAC:
    """V5 Fixed SAC - LayerNorm版本"""
    
    def __init__(self, state_dim=9, action_dim=3,
                 hidden_dims=[768, 768, 512, 512],
                 lr_actor=1e-4, lr_critic=3e-4, lr_alpha=1e-4,
                 gamma=0.998,
                 tau=0.002,
                 grad_clip=1.5,
                 dropout=0.05,
        ...
        # Actor网络
        self.actor = V5FixedActor(state_dim, action_dim, hidden_dims, dropout=dropout).to(self.device)
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=lr_actor, eps=1e-4)
        
        # Twin Critic网络
        self.critic1 = V5FixedCritic(state_dim, action_dim, hidden_dims, dropout=dropout).to(self.device)
        self.critic2 = V5FixedCritic(state_dim, action_dim, hidden_dims, dropout=dropout).to(self.device)
        ...
        # Target网络
        self.critic1_target = V5FixedCritic(state_dim, action_dim, hidden_dims, dropout=dropout).to(self.device)
        self.critic2_target = V5FixedCritic(state_dim, action_dim, hidden_dims, dropout=dropout).to(self.device)
        ...
        # 回放缓冲区
        self.replay_buffer = ReplayBuffer(state_dim, action_dim, capacity=2000000)
    ...
    def _print_network_info(self):
        ...
        print(f"V5Fixed SAC (LayerNorm版):")
...
if __name__ == "__main__":
    print("Testing V5 Fixed SAC (LayerNorm)...")
    sac = V5FixedSAC(state_dim=9, action_dim=3)
    ...
    mean, log_std = sac.actor.forward(state_tensor)
    print(f"Mean shape: {mean.shape}")
    print(f"Log_std shape: {log_std.shape}")
    print("✓ batch_size=1测试通过！")
    ...
    result = sac.update(batch_size=1024)
    print(f"\nUpdate result: {result}")
    
    print("\n✓ V5 Fixed SAC测试通过！LayerNorm工作正常！")
