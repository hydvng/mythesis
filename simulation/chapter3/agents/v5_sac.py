"""
V5 SAC实现 - 针对6500+奖励目标
网络架构: [768, 768, 512, 512] (~4M参数)
关键改进:
1. 更大网络容量
2. 正交初始化
3. 批量归一化替代LayerNorm
...
class V5Actor(nn.Module):
    """
    V5 Actor网络 - 更大容量
    架构: 9 -> 768 -> 768 -> 512 -> 512 -> 3
    """
    ...
    def __init__(self, state_dim, action_dim, 
        ...
        self.input_bn = nn.BatchNorm1d(hidden_dims[0])  # 批量归一化
    ...
    def forward(self, state, use_batch_norm=True):
        # 输入层
        x = self.input_layer(state)
        if use_batch_norm and x.size(0) > 1:
            x = self.input_bn(x)
        x = F.relu(x)
        x = self.dropout(x)
...
class V5Critic(nn.Module):
    """
    V5 Critic网络 (Q网络) - 更大容量
    架构: (9+3) -> 768 -> 768 -> 512 -> 512 -> 1
    """
    ...
    def forward(self, state, action, use_batch_norm=True):
...
class V5SAC:
    ...
    def __init__(self, state_dim=9, action_dim=3,
                 ...
                 gamma=0.998,  # 更高gamma关注长期
                 tau=0.002,
                 grad_clip=1.5,  # 更大的梯度裁剪
                 dropout=0.05,
        ...
        # Actor网络
        self.actor = V5Actor(state_dim, action_dim, hidden_dims, dropout=dropout).to(self.device)
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=lr_actor, eps=1e-4)
        
        # Twin Critic网络
        self.critic1 = V5Critic(state_dim, action_dim, hidden_dims, dropout=dropout).to(self.device)
        self.critic2 = V5Critic(state_dim, action_dim, hidden_dims, dropout=dropout).to(self.device)
        ...
        # Target网络
        self.critic1_target = V5Critic(state_dim, action_dim, hidden_dims, dropout=dropout).to(self.device)
        self.critic2_target = V5Critic(state_dim, action_dim, hidden_dims, dropout=dropout).to(self.device)
    ...
    def _print_network_info(self):
        ...
        print(f"  Replay Buffer: {self.replay_buffer.capacity:,}")
    ...
    def update(self, batch_size=1024):  # 增大batch size
        """更新网络"""
...
if __name__ == "__main__":
    print("Testing V5 SAC...")
    sac = V5SAC(state_dim=9, action_dim=3)
