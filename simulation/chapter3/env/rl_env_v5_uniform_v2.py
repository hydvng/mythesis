"""
第3章：V5.1 风格的 UniformRod 强化学习环境

相对 `rl_env_v5_uniform.py` 的改动：
1. 保留 V5 的纯误差主导思想
2. 增加轻量速度误差项，改善阻尼感与跟踪平滑性
3. 增加软约束惩罚，避免只在发散后才收到惩罚信号
4. 仍保留可选 burst-step 扰动，但默认由训练脚本控制是否启用
"""
...
class PlatformRLEnvV5UniformV2(gym.Env):
    """V5.1 风格 UniformRod 环境。"""
    ...
    def __init__(self,
                 ...
                 vel_error_weight: float = 0.10,
                 constraint_penalty_weight: float = 2.0,
        ...
        self.vel_error_weight = vel_error_weight
    ...
    def _make_history(self):
        return {
            ...
            'vel_error': [], 'constraint_penalty': [], 'error_norm': []
        }
    ...
    def _constraint_penalty(self, q):
        penalty = 0.0
        if q[0] < self.platform.z_min:
            penalty += (self.platform.z_min - q[0]) ** 2
        if q[0] > self.platform.z_max:
            penalty += (q[0] - self.platform.z_max) ** 2
        if abs(q[1]) > self.platform.alpha_max:
            penalty += (abs(q[1]) - self.platform.alpha_max) ** 2
        if abs(q[2]) > self.platform.beta_max:
            penalty += (abs(q[2]) - self.platform.beta_max) ** 2
        return penalty

    def _compute_reward(self, e, ed, q):
        ise = float(np.sum(e ** 2))
        vel_error = float(np.sum(ed ** 2))
        constraint_penalty = float(self._constraint_penalty(q))

        reward = -ise
        reward -= self.vel_error_weight * vel_error
        reward -= self.constraint_penalty_weight * constraint_penalty
        return reward, ise, vel_error, constraint_penalty
    ...
    def step(self, action):
        ...
        reward, ise, vel_error, constraint_penalty = self._compute_reward(e, ed, self.q)
        ...
        self.history['vel_error'].append(vel_error)
        self.history['constraint_penalty'].append(constraint_penalty)
...
if __name__ == '__main__':
    print('Testing V5.1 Uniform RL Environment...')
    env = PlatformRLEnvV5UniformV2(
    ...
    for i in range(100):
        ...
        if i % 20 == 0:
            print(
                f'Step {i}: reward={reward:.4f}, ise={info["ise"]:.6f}, '
                f'vel={info["vel_error"]:.6f}, cpen={info["constraint_penalty"]:.6f}'
            )
        if done:
            break
